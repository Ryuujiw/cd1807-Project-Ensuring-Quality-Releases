...
def login(user, password):
    ...
    print(timestamp() + 'Browser has started. Logging in...')
...
def add_cart(driver, n_items):
    for i in range(n_items):
        ...
        driver.find_element_by_css_selector(element).click()  # Click the found URL
        driver.find_element_by_css_selector("button.btn_primary.btn_inventory").click()  # Add product to the cart
        product = driver.find_element_by_css_selector("div[class='inventory_details_name']").text  # Get the product name
        print(timestamp() + product + " added to shopping cart.")  # Show what product has been added
        driver.find_element_by_css_selector("button.inventory_details_back_button").click()  # Go Back
    print(timestamp() + '{:d} items were added to shopping cart.'.format(n_items))
